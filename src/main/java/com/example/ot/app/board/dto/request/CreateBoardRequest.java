...
public class CreateBoardRequest {
    ...
    @NotBlank(message = "여행위치를 입력해주세요.")
    private Integer regionCode;

    @NotBlank(message = "여행인원을 입력해주세요.")
    @Min(value = 2, message = "여행인원은 2명 이상이어야 합니다.")
    private Integer numberOfTravelers;

    @NotBlank(message = "모집기간을 입력해주세요.")
    @FutureLocalDate(message = "모집기간 시작은 현재 날짜 이후이어야 합니다.")
    private LocalDate recruitmentPeriod;

    @NotBlank(message = "여행기간을 입력해주세요.")
